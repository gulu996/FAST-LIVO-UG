...
public:
  ...
  bool serialReaderRunning() const { return running_.load(); }
  void ingestExternalPvt(const gnss_serial_driver::GnssPvtStamped &message);
  size_t pendingMeasurementCount() const
  {
...
private:
  ...
  void externalPvtCallback(const gnss_serial_driver::GnssPvtStampedConstPtr &message);
  ...
  std::string external_topic_ = "/gnss/pvt_local";
  ...
  int serial_fd_ = -1;
  bool ownership_conflict_ = false;
