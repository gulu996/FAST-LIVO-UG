...
public:
  ...
  bool initialize()
  {
    ...
    const bool uses_shared_log_directory =
        !std::filesystem::path(event_log_file_).is_absolute() ||
        !std::filesystem::path(status_csv_file_).is_absolute();
    bool file_logging_enabled = true;
    if (uses_shared_log_directory)
    {
      ROS_INFO("[GNSS_FAULT] Waiting for the main run log directory.");
      std::string run_directory;
      const auto state = fast_livo_logging::waitForRunLogDirectory(
          node_nh_, run_directory);
      if (state == fast_livo_logging::RunLogDirectoryState::DISABLED)
      {
        file_logging_enabled = false;
        ROS_INFO("[GNSS_FAULT] File output disabled by pcd_save/save_log_en.");
      }
      else if (state != fast_livo_logging::RunLogDirectoryState::READY)
      {
        ROS_ERROR("[GNSS_FAULT] Timed out waiting for %s.",
                  fast_livo_logging::kRunLogDirectoryParam);
        return false;
      }
      else
      {
        event_log_file_ = fast_livo_logging::resolveRunLogFile(
            event_log_file_, run_directory);
        status_csv_file_ = fast_livo_logging::resolveRunLogFile(
            status_csv_file_, run_directory);
      }
    }
    if (file_logging_enabled && event_log_file_ == status_csv_file_)
    {
      ROS_ERROR("[GNSS_FAULT] event_log_file and status_csv_file must differ.");
      return false;
    }
    if (file_logging_enabled &&
        (!openCsv(event_log_file_, event_stream_) ||
    ...
    if (file_logging_enabled)
    {
      ...
      ROS_INFO("[GNSS_FAULT] CSV outputs: %s, %s", event_log_file_.c_str(),
               status_csv_file_.c_str());
    }
