...
bool GnssManager::initialize(ros::NodeHandle &nh, const std::string &save_path)
{
  ...
  if (input_mode_ == "disabled")
  {
    en_ = false;
    convergence_state_ = ConvergenceState::DISABLED;
    ROS_INFO("[GNSS] input_mode=disabled; no serial thread or external subscriber.");
    return false;
  }
  ...
  if (input_mode_ == "external_topic")
  {
    external_subscriber_ = nh.subscribe(external_topic_, 128,
                                         &GnssManager::externalPvtCallback, this);
    transitionTo(ConvergenceState::WAIT_VALID_DATA, 0.0,
                 "WAIT_EXTERNAL_GNSS_LOCAL");
    ROS_INFO("[GNSS] input_mode=external_topic topic=%s; legacy serial is not opened.",
             external_topic_.c_str());
    return true;
  }
  ...
  if (!openSerial())
  {
    if (ownership_conflict_)
    {
      ROS_ERROR("[GNSS] Serial ownership conflict on %s; legacy read thread will not start.",
                serial_port_.c_str());
      return false;
    }
...
bool GnssManager::loadParameters(ros::NodeHandle &nh)
{
  ...
  nh.param<std::string>("gnss/input_mode", input_mode_, input_mode_);
  nh.param<std::string>("gps/external_topic", external_topic_, "/gnss/pvt_local");
  nh.param<std::string>("gnss/external_topic", external_topic_, external_topic_);
  input_mode_ = toLower(input_mode_);
  if (input_mode_ != "legacy_internal" && input_mode_ != "external_topic" &&
      input_mode_ != "disabled")
  {
    ROS_ERROR("[GNSS] Invalid input_mode='%s'.", input_mode_.c_str());
    return false;
  }
...
bool GnssManager::openSerial()
{
  ...
  {
    ownership_conflict_ = true;
    ROS_ERROR("[GNSS] Cannot acquire exclusive serial lock on %s: %s",
  ...
  while (running_.load())
  {
    if (serial_fd_ < 0)
    {
      ...
      if (now - last_reopen_attempt > 1.0)
      {
        ...
        else if (ownership_conflict_)
        {
          ROS_ERROR("[GNSS] Serial ownership conflict after reconnect; stopping legacy reader.");
          running_.store(false);
          break;
        }
      }
...
void GnssManager::externalPvtCallback(
    const gnss_serial_driver::GnssPvtStampedConstPtr &message)
{
  ingestExternalPvt(*message);
}

void GnssManager::ingestExternalPvt(
    const gnss_serial_driver::GnssPvtStamped &message)
{
  if (input_mode_ != "external_topic" || !message.local_measurement_time_valid ||
      !message.valid_for_fusion || message.header.stamp.isZero())
    return;

  GnssMeasurement measurement;
  measurement.seq = static_cast<int>(message.source_sequence & 0x7fffffffULL);
  measurement.stamp = message.header.stamp.toSec();
  measurement.receive_stamp = measurement.stamp;
  measurement.device_stamp = static_cast<double>(message.utc_measurement_ns) * 1e-9;
  measurement.device_time_valid = message.utc_valid;
  measurement.latitude_deg = message.pvt.latitude;
  measurement.longitude_deg = message.pvt.longitude;
  measurement.altitude_m = message.pvt.altitude;
  measurement.velocity_north = message.pvt.vel_n;
  measurement.velocity_east = message.pvt.vel_e;
  measurement.velocity_up = -message.pvt.vel_d;
  measurement.satellite_count = message.pvt.num_sv;
  measurement.horizontal_std_m = message.pvt.h_acc;
  measurement.vertical_std_m = message.pvt.v_acc;
  measurement.hdop = message.pvt.p_dop;
  measurement.raw_position_quality =
      message.pvt.carr_soln == 2 ? 4 : (message.pvt.carr_soln == 1 ? 5 : 1);
  measurement.state = measurement.raw_position_quality;
  measurement.solution_type =
      message.pvt.carr_soln == 2 ? GnssSolutionType::RTK_FIXED :
      (message.pvt.carr_soln == 1 ? GnssSolutionType::RTK_FLOAT :
       (message.pvt.diff_soln ? GnssSolutionType::DIFFERENTIAL :
        GnssSolutionType::SINGLE));
  measurement.source_message = "EXTERNAL_PVT_LOCAL";
  measurement.checksum_valid = true;
  measurement.valid = message.pvt.valid_fix;
  measurement.reject_reason = measurement.valid ? "ok" : "reject_invalid_status";
