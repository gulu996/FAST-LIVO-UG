...
void testRunLogDirectoryProtocol()
{
  using fast_livo_logging::RunLogDirectoryState;
  std::string directory;
  check(fast_livo_logging::classifyRunLogDirectory(
            fast_livo_logging::kRunLogDirectoryPending, &directory) ==
            RunLogDirectoryState::PENDING &&
            directory.empty(),
        "pending run log directory was treated as ready");
  check(fast_livo_logging::classifyRunLogDirectory(
            fast_livo_logging::kRunLogDirectoryDisabled, &directory) ==
            RunLogDirectoryState::DISABLED &&
            directory.empty(),
        "disabled run log directory was treated as ready");
  check(fast_livo_logging::classifyRunLogDirectory(
            "/home/gulu/data/2026-07-22-191535/", &directory) ==
            RunLogDirectoryState::READY &&
            directory == "/home/gulu/data/2026-07-22-191535/",
        "ready run log directory was not preserved");
  check(fast_livo_logging::resolveRunLogFile(
            "gnss_fault_events.csv", directory) ==
            "/home/gulu/data/2026-07-22-191535/gnss_fault_events.csv",
        "relative GNSS log was not placed in the run directory");
  check(fast_livo_logging::resolveRunLogFile(
            "./gnss_fault_events.csv", directory) ==
            "/home/gulu/data/2026-07-22-191535/gnss_fault_events.csv",
        "equivalent relative GNSS log paths were not normalized");
  check(fast_livo_logging::resolveRunLogFile(
            "/data/test/override.csv", directory) ==
            "/data/test/override.csv",
        "absolute test override was unexpectedly rewritten");
}
} // namespace
