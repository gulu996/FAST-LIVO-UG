...
int main(int argc, char **argv)
{
...

  {
    GnssManager manager;
    configureTestManager(manager);
    manager.input_mode_ = "external_topic";
    gnss_serial_driver::GnssPvtStamped external;
    external.header.stamp.fromSec(946684801.25);
    external.source_sequence = 77;
    external.utc_measurement_ns = 1700000000000000000ULL;
    external.utc_valid = true;
    external.local_measurement_time_valid = true;
    external.valid_for_fusion = true;
    external.pvt.valid_fix = true;
    external.pvt.carr_soln = 2;
    external.pvt.latitude = 30.27413;
    external.pvt.longitude = 120.15516;
    external.pvt.altitude = 16.0;
    external.pvt.h_acc = 0.05;
    external.pvt.v_acc = 0.10;
    manager.ingestExternalPvt(external);
    assert(manager.pendingMeasurementCount() == 1);
    assert(manager.measurement_queue_.front().seq == 77);
    assert(std::fabs(manager.measurement_queue_.front().stamp - 946684801.25) < 1e-9);
    assert(manager.measurement_queue_.front().solution_type == GnssSolutionType::RTK_FIXED);
    external.valid_for_fusion = false;
    manager.ingestExternalPvt(external);
    assert(manager.pendingMeasurementCount() == 1);
  }
